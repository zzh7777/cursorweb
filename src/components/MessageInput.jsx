...
const DATA_SOURCES = [
  ...
  { value: '107', label: '107数据源' },
  { value: 'medical', label: '医保数据源' },
];
...
export default function MessageInput({ onSend, disabled, uploading, onStop }) {
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      if (dsRef.current && !dsRef.current.contains(e.target)) setShowDsDropdown(false);
      if (dbRef.current && !dbRef.current.contains(e.target)) setShowDbDropdown(false);
    };
  ...
  return (
    <div className="border-t border-border bg-zinc-950 px-4 py-3">
      <div className="max-w-3xl mx-auto">
        ...
        {/* Data source & DB type selectors */}
        <div className="mb-2 flex items-center gap-2">
          <div ref={dsRef} className="relative">
            <button
              onClick={() => { setShowDsDropdown(!showDsDropdown); setShowDbDropdown(false); }}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium
                         bg-surface border border-border hover:border-primary/50 text-zinc-300
            ...
            >
              ...
              {DATA_SOURCES.find((d) => d.value === dataSource)?.label}
              <svg className={`w-3 h-3 text-zinc-500 transition-transform ${showDsDropdown ? 'rotate-180' : ''}`}
                   fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
              </svg>
            </button>
            {showDsDropdown && (
              <div className="absolute bottom-full mb-1 left-0 w-40 bg-surface border border-border
                              rounded-lg shadow-lg shadow-black/40 overflow-hidden z-50
                              animate-[fadeIn_0.15s_ease-out]">
                {DATA_SOURCES.map((ds) => (
                  <button
                    key={ds.value}
                    onClick={() => { setDataSource(ds.value); setShowDsDropdown(false); }}
          ...
          <div ref={dbRef} className="relative">
            <button
              onClick={() => { setShowDbDropdown(!showDbDropdown); setShowDsDropdown(false); }}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium
                         bg-surface border border-border hover:border-primary/50 text-zinc-300
                         transition-colors cursor-pointer"
            >
              ...
              {DB_TYPES.find((d) => d.value === dbType)?.label}
              <svg className={`w-3 h-3 text-zinc-500 transition-transform ${showDbDropdown ? 'rotate-180' : ''}`}
                   fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
              </svg>
            </button>
            {showDbDropdown && (
              <div className="absolute bottom-full mb-1 left-0 w-36 bg-surface border border-border
                              rounded-lg shadow-lg shadow-black/40 overflow-hidden z-50
                              animate-[fadeIn_0.15s_ease-out]">
                {DB_TYPES.map((db) => (
                  <button
                    key={db.value}
                    onClick={() => { setDbType(db.value); setShowDbDropdown(false); }}
                    className={`w-full text-left px-3 py-2 text-xs transition-colors cursor-pointer
                      ${dbType === db.value
        ...
        >
          ...
          {disabled && !uploading ? (
            <button
              onClick={onStop}
              className="shrink-0 p-2 rounded-lg bg-red-500/80 hover:bg-red-500
                         transition-colors cursor-pointer"
              title="停止生成"
            >
