...
本技能支持"提问→解答→知识沉淀"的迭代机制，通过 `qa-log.md` 和 `knowledge-base.md` 两个文件实现 LLM 能力的持续增长。
...
┌─────────────────────────────────────────────────────┐
│  LLM 处理规则                                        │
│  ├─ 遇到不确定因素 → 追加到 qa-log.md [PENDING]       │
│  └─ 继续处理（标注该不确定因素对输出的影响）            │
└──────────────────────┬──────────────────────────────┘
                       ▼
┌─────────────────────────────────────────────────────┐
│  人工异步解答                                        │
│  ├─ 查看 qa-log.md 中 [PENDING] 的问题               │
│  └─ 填写答案，状态改为 [ANSWERED]                     │
└──────────────────────┬──────────────────────────────┘
                       ▼
┌─────────────────────────────────────────────────────┐
│  LLM 知识沉淀（下次会话触发）                         │
│  ├─ 读取 qa-log.md 中 [ANSWERED] 的问题               │
│  ├─ 提炼通用知识 → 写入 knowledge-base.md             │
│  ├─ 若为高频/关键知识 → 更新 SKILL.md 对应章节        │
│  └─ 原问题标记为 [INCORPORATED]                       │
└─────────────────────────────────────────────────────┘
```
...
1. **规则文本歧义**：存在多种合理解读，且不同解读导致 SQL 逻辑不同
2. **字段语义不确定**：不清楚某字段在特定业务场景下的含义或取值范围
3. **数据质量异常**：发现数据分布与预期不符（如某字段空值率异常高）
4. **业务逻辑缺口**：规则未覆盖的边界情况（如跨年度、跨机构）
5. **计算口径模糊**：违规金额、阈值等计算方式存在多种可能

### 何时沉淀

每次新会话开始处理规则前，先检查 `qa-log.md`：

1. 若有 `[ANSWERED]` 状态的问题 → 立即执行沉淀流程
2. 提炼答案中的通用知识点，按分类写入 `knowledge-base.md`
3. 若该知识属于表结构、踩坑清单等核心范畴 → 同步更新 SKILL.md
4. 将已沉淀的问题标记为 `[INCORPORATED]`

### 沉淀原则

- **提炼而非复制**：不是把答案原文搬到知识库，而是提取可复用的规则/模式
- **附注来源**：每条知识标注来源问题编号（如 `来源: Q-003`）
- **分类归档**：按 knowledge-base.md 中的六大分类放置
- **升级机制**：反复出现的同类问题应升级为 SKILL.md 的正式条目

### 相关文件

- [qa-log.md](qa-log.md) — 问答追踪日志，记录问题与解答
- [knowledge-base.md](knowledge-base.md) — 知识沉淀库，分类存储提炼后的知识
